...
<div class="modal fade mt-3" id="modal-tambah" role="dialog" aria-labelledby="modal-alkes" aria-hidden="true" data-backdrop="static">
            ...
            <div class="col-md-6">
                <div class="form-group">
                  ...
                  <div class="col-sm-12 col-md-12">
                    <input class="form-control" type="text" name="kode_alkes" id="kode_alkes" placeholder="Kode alkes" required>
                  </div>
                </div>
                <div class="form-group">
                  <label class="col-sm-12">Nama alkes</label>
                  <div class="col-sm-12 col-md-12">
                    <input class="form-control" type="text" name="nama_alkes" id="nama_alkes" placeholder="Nama alkes" required>
                  </div>
                </div>
                <div class="form-group">
                  ...
                  <div class="col-md-12 col-sm-12">
                    <select class="form-control col-lg-12 col-md-12 col-12" id="id_kategori" name="id_kategori" required>
                        ...
                        <?php
                        $qktg=$this->db->get('kategori_alkes');
  ...
  <script>
    $('document').ready(function(){
      ...
      table = $('#t_alkes').DataTable({
          ...
          "ajax": {
              "url": "<?php echo site_url('a_alkes/get_alkes')?>",
              "type": "POST"
          },
          "columnDefs": [
          {
      ...
      $("#simpan").click(function(e) {
        ...
        if($('#form').parsley().validate()){
          ...
          $.ajax({
            ...
            cache: false,
            async:true,
            url     : "<?php echo site_url('a_alkes/simpan');?>",
            success : function(data) {
              process_done1(param,'<i class="fa fa-save"></i> Simpan'); 
              if(data=='E202'){
                Swal.fire('Error!','Kode alkes sudah ada. Silahkah gunakan kode yang lain','error');
              }else{
      ...
      $("#tambah").click(function() {
        ...
        $("#deskripsi").html('');
        $('#form')[0].reset(); 
        $('#kode_alkes').prop('readonly',false);
        $('.modal-load').hide(); 
      });

      $('#t_alkes').on( 'click', 'tr .edit', function () {
        $('.modal-load').show();
        $('#form').parsley().reset();
          var id = $(this).data("id");
          $.ajax({
            type    : 'POST',
            data    : {id:id},
            url     : "<?php echo site_url('a_alkes/cari_alkes');?>",
            dataType: 'json',
            success : function(data) {
              $('#id').val(data.id);
              $('#nama_alkes').val(data.nama_alkes);
              $('#kode_alkes').val(data.kode_alkes);
              $('#kode_alkes').prop('readonly',true);
      ...
      $('#t_alkes').on( 'click', 'tr .delete', function () {
          var id = $(this).data("id");
          Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type    : 'POST',
                        data    : {id:id},
                        url     : "<?php echo site_url('a_alkes/hapus');?>",
                        success : function(data) {
